...
public class Presenter {
    ...
    public boolean compareData() {
        List<Animal> originalData = new LinkedList<>();
    ...
    public void teachAnimalNewCommand(String command, int index) {
        dataBase.getAnimals().get(index).learnNewCommands(command);
    }
    ...
    /**
     * Считка информации из локальной БД (текстовый файл с объектами
     * в json-формате) в буфер (список объектов)
     */
    public void fillListFromLocalDB(List<Animal> list) {
        list.clear();
        try
